...
대한민국 현행 법령 문서를 기반으로 사용자의 법률 관련 질문에 대해 관련 조문을 검색하고, 이를 근거로 답변을 생성하는 RAG 시스템입니다.

기존 Dense Retriever 중심 구조에서 발생한 키워드 누락, 조문 혼입, 근거 불일치 문제를 개선하기 위해 BM25 기반 키워드 검색을 추가하고, Dense Search와 BM25 Search를 RRF 방식으로 결합한 Hybrid Search 구조를 적용했습니다.

최종적으로는 Hybrid Search로 후보 문서를 확보하고, Reranker로 관련도를 재정렬한 뒤, Local LLM을 통해 법적 근거가 포함된 답변을 생성하는 파이프라인을 구성했습니다.
...
이 프로젝트의 목표는 단순히 LLM으로 법률 답변을 생성하는 것이 아니라, 답변의 근거가 되는 법령 조문을 정확히 검색하는 것입니다.

법령 문서는 일반 문서와 달리 법령명, 조문 번호, 금액, 기간, 처벌 기준 등 정확한 표현이 중요합니다. 따라서 의미 기반 검색만으로는 실제 답변에 필요한 정답 조문을 안정적으로 찾기 어려울 수 있습니다.

이를 해결하기 위해 Dense Search와 BM25 Search를 결합한 Hybrid Search 구조를 적용했습니다.
...
법령 RAG에서는 답변 내용뿐 아니라 어떤 조문을 근거로 답변했는지가 중요하기 때문에, 검색 결과의 정확도와 답변의 조문 인용 정확도를 분리해서 평가할 필요가 있었습니다.
...
Dense Search는 jinaai/jina-embeddings-v3를 사용해 조문을 벡터화하고, ChromaDB에 저장한 뒤 유사도 검색을 수행했습니다.
...
* 의미 기반 검색에 강함
* 질문 표현과 법령 표현이 달라도 유사한 조문을 찾을 수 있음
...
BM25는 질문에 포함된 핵심 키워드가 문서에 직접 등장하는 경우 강점을 보입니다. 특히 법령명, 제도명, 처벌 기준, 금액, 기간처럼 정확한 표현이 중요한 질문에서 유용했습니다.
...
여기서 rank_i(d)는 각 검색기에서 문서 d가 가진 순위이고, k는 상위 문서에 과도하게 점수가 몰리는 것을 완화하기 위한 상수입니다.
...
최종 검색 결과에서 도로교통법 제12조 어린이 보호구역의 지정ㆍ해제 및 관리가 1위로 검색되었습니다.
...
Dense Search는 의미 기반 후보를 확보했고, BM25는 어린이 보호구역, 속도, 30km와 같은 키워드 기반 후보를 보완했습니다. 이후 Reranker를 통해 핵심 조문이 최종 상위에 유지되었습니다.

⸻

12.2 성공 사례: 주차위반 차량 견인
...
최종 검색 결과에서 도로교통법 시행령 제13조 주차위반 차의 견인ㆍ보관 및 반환 등을 위한 조치와 도로교통법 시행규칙 제22조 주차위반차의 견인ㆍ보관 및 반환 등을 위한 조치 등이 상위에 배치되었습니다.
...
사용자 표현인 휴대폰과 법령 표현인 이동전화, 영상표시장치 등이 제대로 매핑되지 않아 직접 근거 조문 검색에 실패한 것으로 분석했습니다.
...
검색 결과에는 어린이 보호구역 제한속도와 관련된 도로교통법 제12조가 포함되었습니다.
...
질문 유형이 규정 확인형인지 위반 처분형인지 구분되지 않아, 제한속도 조문은 검색되었지만 처분 기준 문서가 충분히 검색되지 않았습니다.
...
한계	개선 방향
사용자 표현과 법령 표현 차이	법령 도메인 동의어 사전 구축
질문 유형 구분 부족	규정 확인형, 위반 처분형, 신고 절차형 분류
무관 문서 혼입	Reranker 이후 rule-based filtering 추가
조문 번호 오인용	metadata 기반 조문 번호 자동 삽입
LLM 과잉 생성	검색 결과에 근거가 없을 경우 단정하지 않도록 프롬프트 강화
긴 조문 내부 세부 조건 검색 어려움	항·호 단위 chunking 검토
정량 평가 부족	Recall@K, MRR, Precision@K 도입
법령 외 실무 절차 답변 한계	판례, 행정해석, 고시, 신고 절차 문서 보강

향후에는 질문 유형에 따라 Dense Search와 BM25 Search의 비중을 조절하는 Adaptive Hybrid Search도 검토할 수 있습니다.
...
LM Studio 서버를 실행한 뒤, 다음과 같이 API 주소를 설정합니다.
